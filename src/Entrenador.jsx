...
// ============================================================================
// ENTRENAMÁTICO v13 · Pensamiento Matemático I-VI + CNEyT I-VI (MCCEMS 2025)
// Compañero digital de los 12 cuadernillos extendidos del ecosistema.
// ACTUALIZACIÓN v13 (6 jul 2026): Renombrado de "Entrenador" a "Entrenamático"
...
const GEN_CNEYT6 = {
  ...
  PF2: (nv) => {
    const { opciones, correcta } = armar("Robert Hooke", ["Charles Darwin", "Gregor Mendel", "Isaac Newton"]);
    return { texto: `¿Quién acuñó el término "célula" en 1665 tras observar un corte de corcho al microscopio?`, opciones, correcta, explica: `Fue Robert Hooke, en su obra Micrographia, el primero en usar el término "célula" en biología.` };
  },
  PF3: (nv) => {
...
function PlotSVG({ spec }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", maxWidth: 300, display: "block", margin: "10px auto", background: "#FBF7EA", border: "1px solid #d9cfae", borderRadius: 8 }}>
      {grid}
      <line x1={X(xmin)} y1={Y(0)} x2={X(xmax)} y2={Y(0)} stroke="#8a7a5c" strokeWidth="1.5" />
      <line x1={X(0)} y1={Y(ymin)} x2={X(0)} y2={Y(ymax)} stroke="#8a7a5c" strokeWidth="1.5" />
      <path d={d} fill="none" stroke="#3D6B35" strokeWidth="2.5" />
      {(spec.points || []).map((p, i) => (
        <circle key={i} cx={X(p.x)} cy={Y(p.y)} r="4.5" fill="#D9A526" stroke="#2E2A21" strokeWidth="1.2" />
      ))}
    </svg>
  );
}
  ...
  for (let i = 0; i < etapa * 2; i++) {
    ...
    hojas.push(
      <path key={i}
        ...
        fill={i >= etapa * 2 - 2 ? "#6FA05C" : "#3D6B35"} opacity="0.95" />
    );
  }
  return (
    <svg viewBox="0 0 100 100" style={{ width: 92, height: 92 }} aria-label={`Milpa, racha ${racha}`}>
      ...
      <rect x="47.5" y={92 - 12 - etapa * 13} width="5" height={12 + etapa * 13} rx="2.5" fill="#3D6B35" />
      {hojas}
      {etapa >= 4 && <ellipse cx="50" cy={92 - 16 - etapa * 13} rx="7" ry="12" fill="#D9A526" stroke="#B08650" strokeWidth="1.5" />}
...
const CI = { milpaD: "#2F5233", milpa: "#3D6B35", milpaS: "#EAF1E3", maiz: "#D9A526", maizS: "#FBF3DC", ink: "#2E2A21", muted: "#8a7a5c", surco: "#6B4F2A", papel2: "#FFFDF6", linea: "#cabf9d", rojo: "#B4432E", azul: "#355070" };
...
function BalanzaEcuacion() {
  ...
  const Pan = ({ xCount, uCount, side }) => (
    <div style={{
      minWidth: 128, minHeight: 74, background: "#FFFDF6", border: "2px solid #2E2A21", borderRadius: 10,
      ...
      {Array.from({ length: xCount }).map((_, i) => <Block key={"x" + i} label="x" color="#355070" />)}
      {Array.from({ length: uCount }).map((_, i) => <Block key={"u" + i} label="1" color="#3D6B35" />)}
      {xCount === 0 && uCount === 0 && <span style={{ color: "#8a7a5c", fontSize: 11, padding: 4 }}>vacío</span>}
    </div>
  );
  const notas = [
  ...
  return (
    <div>
      <div style={{ textAlign: "center", fontSize: 17, fontWeight: 800, color: "#2F5233", marginBottom: 4 }}>
      ...
      <div style={{ display: "flex", justifyContent: "center", alignItems: "flex-end", gap: 8, margin: "14px 0 6px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
          ...
          <div style={{ fontSize: 11, color: "#8a7a5c", marginTop: 4 }}>lado izquierdo</div>
        </div>
        <div style={{ fontSize: 22, fontWeight: 800, color: "#2E2A21", marginBottom: 30 }}>=</div>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
          <Pan xCount={0} uCount={rightUnits} side="right" />
          <div style={{ fontSize: 11, color: "#8a7a5c", marginTop: 4 }}>lado derecho</div>
        </div>
      </div>
      <div style={{ background: "#E7ECF3", borderLeft: "4px solid #355070", borderRadius: 8, padding: "9px 12px", fontSize: 13, color: "#2E2A21", margin: "10px 0" }}>
      ...
      <p style={{ fontSize: 12.5, color: "#8a7a5c", fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
...
function RectaInteractiva() {
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", background: "#FFFDF6", borderRadius: 10, border: "1.5px solid #2E2A21" }}>
        {[-6, -4, -2, 2, 4, 6].map((g) => <line key={"v" + g} x1={X(g)} y1={Y(ymin)} x2={X(g)} y2={Y(ymax)} stroke="#EDE6D2" strokeWidth="0.8" />)}
        {[-8, -4, 4, 8].map((g) => <line key={"h" + g} x1={X(xmin)} y1={Y(g)} x2={X(xmax)} y2={Y(g)} stroke="#EDE6D2" strokeWidth="0.8" />)}
        <line x1={X(xmin)} y1={Y(0)} x2={X(xmax)} y2={Y(0)} stroke="#8a7a5c" strokeWidth="1.2" />
        <line x1={X(0)} y1={Y(ymin)} x2={X(0)} y2={Y(ymax)} stroke="#8a7a5c" strokeWidth="1.2" />
        <line x1={X(x1)} y1={Y(f(x1))} x2={X(x2)} y2={Y(f(x2))} stroke="#3D6B35" strokeWidth="2.6" />
        {/* triángulo pendiente rise/run */}
        {Math.abs(y0) <= ymax && Math.abs(y1r) <= ymax && (
          <>
            <line x1={X(x0)} y1={Y(y0)} x2={X(x0 + run)} y2={Y(y0)} stroke="#D9A526" strokeWidth="1.6" strokeDasharray="3 2" />
            <line x1={X(x0 + run)} y1={Y(y0)} x2={X(x0 + run)} y2={Y(y1r)} stroke="#D9A526" strokeWidth="1.6" strokeDasharray="3 2" />
        ...
        {Math.abs(b) <= ymax && <circle cx={X(0)} cy={Y(b)} r="5" fill="#B4432E" stroke="#fff" strokeWidth="1.3" />}
      </svg>
      <div style={{ textAlign: "center", fontSize: 15, fontWeight: 800, color: "#2F5233", margin: "8px 0" }}>
        y = {m}x {b >= 0 ? "+ " + b : "− " + Math.abs(b)}
      </div>
      <label style={{ display: "block", fontSize: 12.5, color: "#8a7a5c", marginBottom: 8 }}>Pendiente (m): <b style={{ color: "#2E2A21" }}>{m}</b>
        <input type="range" min={-4} max={4} step={1} value={m} onChange={(e) => setM(+e.target.value)} style={{ width: "100%", accentColor: "#3D6B35" }} /></label>
      <label style={{ display: "block", fontSize: 12.5, color: "#8a7a5c" }}>Ordenada al origen (b): <b style={{ color: "#2E2A21" }}>{b}</b>
        <input type="range" min={-6} max={6} step={1} value={b} onChange={(e) => setB(+e.target.value)} style={{ width: "100%", accentColor: "#3D6B35" }} /></label>
      <p style={{ fontSize: 12.5, color: "#8a7a5c", fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        <b style={{ color: "#B4432E" }}>b</b> (rojo) es donde la recta cruza el eje y. <b style={{ color: "#B08650" }}>m</b> es cuánto sube (o baja) la recta por cada paso a la derecha — el triángulo amarillo lo muestra.
      </p>
    </div>
  );
}
...
function TraductorAlgebraico() {
  ...
  return (
    <div>
      <div style={{ background: "#FFFDF6", border: "1.5px solid #2E2A21", borderRadius: 10, padding: "16px", textAlign: "center" }}>
        <div style={{ fontSize: 14, color: "#8a7a5c", marginBottom: 6 }}>«{item.frase}»</div>
        <div style={{ fontSize: 22, fontWeight: 800, color: "#355070", margin: "10px 0" }}>{item.expr}</div>
        <div style={{ display: "flex", justifyContent: "center", gap: 14, alignItems: "center", fontSize: 15 }}>
          <span>con n = <b style={{ color: "#2E2A21" }}>{n}</b></span>
          <span style={{ color: "#8a7a5c" }}>→</span>
          <span style={{ background: "#EAF1E3", padding: "4px 12px", borderRadius: 8, fontWeight: 800, color: "#2F5233" }}>{item.expr.replace(/n/g, n)} = {item.f(n)}</span>
        </div>
      </div>
      <label style={{ display: "block", fontSize: 12.5, color: "#8a7a5c", margin: "12px 0 6px" }}>Cambia el número (n): <b style={{ color: "#2E2A21" }}>{n}</b>
        <input type="range" min={0} max={20} value={n} onChange={(e) => setN(+e.target.value)} style={{ width: "100%", accentColor: "#3D6B35" }} /></label>
...
function ClasificadorExpresion() {
  ...
  const colorClase = { monomio: "#355070", binomio: "#3D6B35", trinomio: "#B08650", polinomio: "#B4432E" }[clase];
  return (
    <div>
      <div style={{ background: "#FFFDF6", border: "1.5px solid #2E2A21", borderRadius: 10, padding: "16px", textAlign: "center" }}>
        <div style={{ fontSize: 20, fontWeight: 800, color: "#2E2A21", marginBottom: 10 }}>
          {terminos.map((t, i) => (
            <span key={i}>{i > 0 && <span style={{ color: "#8a7a5c" }}> + </span>}<span style={{ background: "#FBF3DC", padding: "2px 6px", borderRadius: 5 }}>{t}</span></span>
          ))}
        </div>
...
function ModeloAreaMonomios() {
  ...
  return (
    <div>
      <div style={{ textAlign: "center", fontSize: 16, fontWeight: 800, color: "#2F5233", marginBottom: 10 }}>
        (a x)(b) = ab · x &nbsp;→&nbsp; con a={a}, b={b}: área = {a * b}
      </div>
      <div style={{ display: "flex", justifyContent: "center" }}>
        <svg viewBox={`0 0 260 200`} style={{ width: 260, height: 200 }}>
          <rect x={40} y={20} width={a * px} height={b * px} fill="#FBF3DC" stroke="#2E2A21" strokeWidth="1.5" />
          ...
          <text x={40 + (a * px) / 2} y={14} fontSize="11" textAnchor="middle" fill="#6B4F2A">{a} (largo)</text>
          <text x={30} y={20 + (b * px) / 2} fontSize="11" textAnchor="middle" fill="#6B4F2A" transform={`rotate(-90 30 ${20 + (b * px) / 2})`}>{b} (ancho)</text>
          <text x={40 + (a * px) / 2} y={20 + (b * px) / 2 + 4} fontSize="14" fontWeight="800" textAnchor="middle" fill="#2F5233">{a * b}</text>
        </svg>
      </div>
      <label style={{ display: "block", fontSize: 12.5, color: "#8a7a5c", marginTop: 4 }}>a: <b style={{ color: "#2E2A21" }}>{a}</b>
        <input type="range" min={1} max={7} value={a} onChange={(e) => setA(+e.target.value)} style={{ width: "100%", accentColor: "#3D6B35" }} /></label>
      <label style={{ display: "block", fontSize: 12.5, color: "#8a7a5c" }}>b: <b style={{ color: "#2E2A21" }}>{b}</b>
        <input type="range" min={1} max={5} value={b} onChange={(e) => setB(+e.target.value)} style={{ width: "100%", accentColor: "#3D6B35" }} /></label>
      <p style={{ fontSize: 12.5, color: "#8a7a5c", fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
      ...
      <div style={{ display: "flex", justifyContent: "center" }}>
        <svg viewBox="0 0 240 240" style={{ width: 220, height: 220 }}>
          <g transform="translate(30,20)">
            <rect x={0} y={0} width={a * px} height={a * px} fill="#EAF1E3" stroke="#2E2A21" strokeWidth="1.3" />
            <text x={(a * px) / 2} y={(a * px) / 2 + 4} fontSize="12" textAnchor="middle" fontWeight="700" fill="#2F5233">a² = {a * a}</text>
            <rect x={a * px} y={0} width={b * px} height={a * px} fill="#FBF3DC" stroke="#2E2A21" strokeWidth="1.3" />
            <text x={a * px + (b * px) / 2} y={(a * px) / 2 + 4} fontSize="11" textAnchor="middle" fontWeight="700" fill="#B08650">ab={a * b}</text>
            <rect x={0} y={a * px} width={a * px} height={b * px} fill="#FBF3DC" stroke="#2E2A21" strokeWidth="1.3" />
            <text x={(a * px) / 2} y={a * px + (b * px) / 2 + 4} fontSize="11" textAnchor="middle" fontWeight="700" fill="#B08650">ab={a * b}</text>
            <rect x={a * px} y={a * px} width={b * px} height={b * px} fill="#F6E3DE" stroke="#2E2A21" strokeWidth="1.3" />
            <text x={a * px + (b * px) / 2} y={a * px + (b * px) / 2 + 4} fontSize="12" textAnchor="middle" fontWeight="700" fill="#B4432E">b² = {b * b}</text>
            <text x={(a * px) / 2} y={-6} fontSize="10" textAnchor="middle" fill="#8a7a5c">a</text>
            <text x={a * px + (b * px) / 2} y={-6} fontSize="10" textAnchor="middle" fill="#8a7a5c">b</text>
          </g>
        </svg>
      </div>
      <label style={{ display: "block", fontSize: 12.5, color: "#8a7a5c", marginTop: 4 }}>a: <b style={{ color: "#2E2A21" }}>{a}</b>
        <input type="range" min={1} max={6} value={a} onChange={(e) => setA(+e.target.value)} style={{ width: "100%", accentColor: "#3D6B35" }} /></label>
      <label style={{ display: "block", fontSize: 12.5, color: "#8a7a5c" }}>b: <b style={{ color: "#2E2A21" }}>{b}</b>
        <input type="range" min={1} max={5} value={b} onChange={(e) => setB(+e.target.value)} style={{ width: "100%", accentColor: "#3D6B35" }} /></label>
      <p style={{ fontSize: 12.5, color: "#8a7a5c", fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
...
function IdentidadOEcuacion() {
  ...
  return (
    <div>
      <div style={{ textAlign: "center", fontSize: 19, fontWeight: 800, color: "#2E2A21", marginBottom: 12 }}>{item.texto}</div>
      ...
      {probadas.length > 0 && (
        <div style={{ background: "#FFFDF6", border: "1.5px solid #2E2A21", borderRadius: 10, padding: 10 }}>
          {probadas.map((x) => {
            ...
            return (
              <div key={x} style={{ display: "flex", justifyContent: "space-between", fontSize: 13, padding: "4px 2px", borderBottom: "1px solid #EDE6D2" }}>
                ...
                <span style={{ color: ok ? "#3D6B35" : "#B4432E", fontWeight: 800 }}>{ok ? "✓ igual" : "✗ distinto"}</span>
              </div>
            );
          })}
        </div>
      )}
      <div style={{ textAlign: "center", marginTop: 10, fontSize: 13, fontWeight: 700, color: item.tipo === "identidad" ? "#3D6B35" : "#B08650" }}>
...
function TasaVariacion() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La tasa de variación promedio es la pendiente de la recta <b style={{ color: CI.rojo }}>secante</b> (roja) que une los dos puntos. Cuando acercas x₂ a x₁, esa secante se convierte en la recta tangente: ahí nace la derivada.
      </p>
    </div>
  );
}
...
function DerivadaPotencia() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La derivada f'(x)=2x te da la pendiente de la recta <b style={{ color: CI.rojo }}>tangente</b> (roja) en cada punto. Muévete: donde la parábola baja, la pendiente es negativa; en el fondo (x=0) es 0; donde sube, es positiva.
      </p>
    </div>
  );
}
...
function Optimizacion() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        Con {P} m de cerca, el área en función del lado es una parábola invertida. Su punto más alto (donde la derivada vale 0) está en x = {xOpt}: un cuadrado. Mueve el deslizador y verás que ningún otro rectángulo encierra más área.
      </p>
    </div>
  );
}
...
function VennConjuntos() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La unión no es simplemente |A|+|B|: los elementos en el centro (A∩B) se contarían dos veces. Por eso se restan una vez: |A∪B| = |A| + |B| − |A∩B|.
      </p>
    </div>
  );
}
...
function CampanaNormal() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La media (μ) mueve la campana de izquierda a derecha; la desviación (σ) la hace más ancha o más angosta. La forma acampanada siempre se conserva: es el patrón más común en la naturaleza.
      </p>
    </div>
  );
}
...
function EnergiaCinetica() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La energía cinética crece con el <b>cuadrado</b> de la velocidad: al duplicar v, la energía se cuadruplica (por eso frenar a alta velocidad es tan difícil). La masa la aumenta de forma proporcional.
      </p>
    </div>
  );
}
...
function EscalasTemperatura() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        Las tres escalas miden lo mismo con distinto punto de partida y tamaño de grado. Fahrenheit: F = 9/5·C + 32. Kelvin (absoluta, sin negativos): K = C + 273.15. El agua se congela a 0°C = 32°F = 273 K.
      </p>
    </div>
  );
}
...
function EscalaPH() {
  ...
  const color = ph < 7 ? "#B4432E" : ph > 7 ? "#355070" : "#3D6B35";
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La escala de pH va de 0 a 14. Por debajo de 7 la sustancia es ácida; por encima, básica; justo en 7 es neutra (como el agua pura). Cada unidad representa 10 veces más acidez o basicidad.
      </p>
    </div>
  );
}
...
function SegundaLeyNewton() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La segunda ley de Newton (F = m·a) dice que la aceleración es la fuerza dividida entre la masa. Más fuerza → más aceleración (flecha más larga); más masa → menos aceleración para la misma fuerza.
      </p>
    </div>
  );
}
...
function OndaInteractiva() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La velocidad de una onda es su longitud de onda (λ, qué tan separadas están las crestas) por su frecuencia (f, cuántas pasan por segundo). Ondas más juntas (λ pequeña) a la misma velocidad implican mayor frecuencia.
      </p>
    </div>
  );
}
...
function LeyOhm() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La ley de Ohm (V = I·R) relaciona voltaje, corriente y resistencia. Más corriente o más resistencia requieren más voltaje. Los puntos rojos representan la corriente: más amperios, más rápido fluye.
      </p>
    </div>
  );
}
...
function CuadroPunnett() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        El cuadro de Punnett combina un alelo de cada progenitor en cada celda. Con Aa × Aa aparece la clásica proporción 3:1 — 3 con rasgo dominante (contienen A) y 1 recesivo (aa). Cambia los progenitores y observa cómo cambian las proporciones.
      </p>
    </div>
  );
}
const cellHdr = { width: 34, height: 34, textAlign: "center", fontWeight: 800, color: "#8a7a5c", fontSize: 15 };
function cell(v) { return { width: 44, height: 44, textAlign: "center", fontWeight: 800, border: "1.5px solid #2E2A21", background: v === "aa" ? "#F6E3DE" : "#EAF1E3", color: v === "aa" ? "#B4432E" : "#2F5233" }; }
        ...
        <tbody>
          {filas.map(([p, q], i) => { const r = cur.f(p, q); return (
            <tr key={i}>
              ...
              <td style={{ ...tdC, background: r ? "#EAF1E3" : "#F6E3DE", fontWeight: 800, color: r ? CI.milpaD : CI.rojo }}>{r ? "V" : "F"}</td>
            </tr>
          ); })}
        </tbody>
      </table>
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        Cambia de operador arriba y observa cómo cambia el patrón de verdaderos y falsos. Ese patrón fijo, no la intuición, es lo que define a cada operador lógico.
      </p>
    </div>
  );
}
...
function ValorPosicional() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        En el sistema decimal, cada posición vale 10 veces más que la de su derecha. El mismo dígito representa cantidades muy distintas según en qué lugar esté: eso es el valor posicional.
      </p>
    </div>
  );
}
...
function RectaEnteros() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        Sumar un entero es dar un salto sobre la recta numérica: a la derecha si es positivo, a la izquierda si es negativo. El punto final (rojo) es el resultado de encadenar ambos saltos desde el 0.
      </p>
    </div>
  );
}
...
function PorcentajeFraccion() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        Toda fracción es una proporción con base 100 en disfraz: dividir el numerador entre el denominador y multiplicar por 100 te da el porcentaje equivalente.
      </p>
    </div>
  );
}
...
function CrecimientoPotencias() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        Cada barra es la base multiplicada por sí misma una vez más que la anterior. Subir el exponente en 1 no suma: multiplica de nuevo por la base, por eso el crecimiento se acelera tan rápido.
      </p>
    </div>
  );
}
...
function NotacionCientifica() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        Mueve el exponente hacia arriba y verás números astronómicamente grandes; hacia abajo (negativo), infinitesimalmente pequeños. La mantisa siempre se mantiene entre 1 y 10.
      </p>
    </div>
  );
}
...
function JerarquiaOperaciones() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        La jerarquía siempre resuelve en el mismo orden: paréntesis → potencias/raíces → multiplicación/división → suma/resta. Avanza paso a paso y comprueba que no hay ambigüedad si sigues ese orden.
      </p>
    </div>
  );
}
...
function SistemaDosRectas() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 6, lineHeight: 1.5 }}>
        La solución de un sistema 2×2 es el punto donde ambas rectas se cruzan: el único (x, y) que cumple las dos ecuaciones a la vez. Si las pendientes son iguales, las rectas son paralelas y no hay solución.
      </p>
    </div>
  );
}
...
function FactorizacionCuadratica() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        Elegir las raíces (puntos naranja donde la parábola cruza el eje x) y expandir (x−r1)(x−r2) es el camino inverso a factorizar: de las raíces a la ecuación general.
      </p>
    </div>
  );
}
...
function InteresSimpleCompuesto() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        Con pocos años la diferencia es chica; sube los años y verás cómo el interés compuesto se dispara: cada periodo gana interés también sobre el interés ya acumulado, no solo sobre el capital original.
      </p>
    </div>
  );
}
...
function TeoremaPitagoras() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        En cualquier triángulo rectángulo, el cuadrado de la hipotenusa (c) es igual a la suma de los cuadrados de los catetos. Mueve a y b y comprueba que la relación se mantiene siempre.
      </p>
    </div>
  );
}
...
function DistanciaPuntos() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 6, lineHeight: 1.5 }}>
        La línea punteada roja (la distancia) es la hipotenusa de un triángulo rectángulo formado por las diferencias en x y en y (líneas azules) — por eso la fórmula de distancia es, en el fondo, el teorema de Pitágoras.
      </p>
    </div>
  );
}
...
function CirculoUnitario() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        En el círculo unitario (radio 1), el coseno es la proyección horizontal (azul) y el seno la vertical (roja) del punto marcado. Gira el ángulo y observa cómo ambos valores oscilan entre −1 y 1.
      </p>
    </div>
  );
}
...
function CoordenadasPolares() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        Un punto puede ubicarse por su distancia al origen y un ángulo (polares) o por sus coordenadas x, y (cartesianas). Ambas describen el mismo punto — son dos idiomas distintos para la misma ubicación.
      </p>
    </div>
  );
}
...
function EcuacionRecta() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        En y = mx + b, m controla la inclinación de la recta (positiva sube, negativa baja) y b (punto rojo) es dónde cruza el eje vertical. Cambia ambos y observa cómo la recta gira y se desplaza.
      </p>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", background: CI.papel2, borderRadius: 10, border: `1.5px solid ${CI.ink}` }}>
        <line x1={X(xmin)} y1={Y(0)} x2={X(xmax)} y2={Y(0)} stroke={CI.muted} strokeWidth="1" />
        <line x1={X(0)} y1={Y(ymin)} x2={X(0)} y2={Y(ymax)} stroke={CI.muted} strokeWidth="1" />
        <circle cx={X(h)} cy={Y(k)} r={r * escala} fill={CI.milpaS} opacity="0.35" stroke={CI.milpaD} strokeWidth="2" />
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 6, lineHeight: 1.5 }}>
        Toda circunferencia con centro (h,k) y radio r cumple (x−h)² + (y−k)² = r². Es la misma idea que usa Kepler para las órbitas: una curva donde todo punto guarda una relación fija con un centro.
      </p>
    </div>
  );
}
...
function SeccionesConicas() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        Todas las cónicas nacen de cortar un cono con un plano. La excentricidad describe qué tan "estirada" está la curva: e=0 círculo perfecto, 0&lt;e&lt;1 elipse, e=1 parábola, e&gt;1 hipérbola.
      </p>
    </div>
  );
}
...
function PosicionTiempo() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La velocidad instantánea es la pendiente de la recta tangente (roja) a la curva posición-tiempo en ese instante exacto — no la velocidad promedio de todo el trayecto, sino la de ese momento preciso.
      </p>
    </div>
  );
}
...
function SimetriaFuncion() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        Una función es par si f(−x)=f(x) (simétrica al eje y); impar si f(−x)=−f(x) (simétrica al origen). Cambia de función y observa el patrón visual de cada tipo.
      </p>
    </div>
  );
}
...
function ConceptoLimite() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La función no está definida exactamente en x=3 (dividir entre 0), pero se acerca cada vez más a 6 mientras más te aproximas. Ese valor al que se acerca —sin necesidad de llegar— es el límite.
      </p>
    </div>
  );
}
...
function FuncionesTrascendentes() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        Las funciones trascendentes no se resuelven con álgebra elemental. La exponencial crece cada vez más rápido; el logaritmo es su inversa; las trigonométricas (seno, coseno) se repiten en ciclos.
      </p>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", background: CI.papel2, borderRadius: 10, border: `1.5px solid ${CI.ink}` }}>
        ...
        <line x1={X(0)} y1={Y(0)} x2={X(6)} y2={Y(6)} stroke={CI.milpa} strokeWidth="2.2" />
      </svg>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        El área sombreada bajo f(x)=x, de 0 a b, es exactamente b²/2 — la antiderivada evaluada en los límites. Así conecta el Teorema Fundamental del Cálculo la derivada con el área acumulada.
      </p>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 10, justifyContent: "center", marginBottom: 12 }}>
        <button className="tab" style={{ padding: "10px 16px", background: resp === "determinista" ? (cur.t === "determinista" ? "#EAF1E3" : "#F6E3DE") : "#fff" }} onClick={() => elegir("determinista")}>Determinista</button>
        <button className="tab" style={{ padding: "10px 16px", background: resp === "aleatorio" ? (cur.t === "aleatorio" ? "#EAF1E3" : "#F6E3DE") : "#fff" }} onClick={() => elegir("aleatorio")}>Aleatorio</button>
      </div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 12, lineHeight: 1.5 }}>
        Un evento determinista siempre da el mismo resultado bajo las mismas condiciones; uno aleatorio no se puede predecir con certeza, aunque se repita en condiciones iguales.
      </p>
    </div>
  );
}
...
function SimulacionDado() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        Con pocos lanzamientos las barras son desiguales, pero mientras más lanzas, más se acercan todas al 16.7% (1/6) esperado teóricamente. Esa es la diferencia entre probabilidad teórica y frecuencia observada.
      </p>
    </div>
  );
}
...
function ConteoPermutaciones() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        La permutación cuenta el orden (1º, 2º lugar); la combinación no. Por eso las combinaciones siempre son menos o igual que las permutaciones: se agrupan los casos que solo difieren en el orden.
      </p>
    </div>
  );
}
...
function TipoGrafica() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        La gráfica correcta depende del tipo de dato: categorías → barras; datos numéricos continuos agrupados → histograma; evolución en el tiempo → líneas.
      </p>
    </div>
  );
}
...
function CorrelacionDispersión() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        Cuando los puntos se acomodan cerca de una línea, la correlación es fuerte; cuando están dispersos sin patrón, es débil o nula. Correlación no significa que una variable cause a la otra — solo que se mueven juntas.
      </p>
    </div>
  );
}
...
function PoblacionMuestra() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        En el muestreo aleatorio simple, cada miembro de la población (puntos grises) tiene la misma probabilidad de quedar en la muestra (puntos rojos). A mayor muestra, mejor representa a la población completa.
      </p>
    </div>
  );
}
...
function CienciaSocial() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        La ciencia avanza en comunidad: hipótesis que se discuten, se revisan por pares y se corrigen entre muchas personas a lo largo del tiempo — no por la iluminación repentina de una sola mente aislada.
      </p>
    </div>
  );
}
...
function FenomenosInterrelacionados() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        Un mismo fenómeno natural rara vez pertenece a una sola disciplina: física, química y biología ofrecen ángulos distintos de la misma realidad interconectada.
      </p>
    </div>
  );
}
...
function DensidadObjetos() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La densidad (masa/volumen) determina si un objeto flota o se hunde en un líquido: si es menor que la densidad del líquido, flota; si es mayor, se hunde — sin importar cuánto pese en total.
      </p>
    </div>
  );
}
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "center", marginBottom: 10, flexWrap: "wrap" }}>
        {["mezcla", "elemento", "compuesto"].map((t) => (
          <button key={t} className="tab" style={{ padding: "8px 12px", background: resp === t ? (cur.t === t ? "#EAF1E3" : "#F6E3DE") : "#fff" }} onClick={() => setResp(t)}>{t}</button>
        ))}
      </div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 12, lineHeight: 1.5 }}>
        Elemento: una sola clase de átomo. Compuesto: átomos distintos unidos en proporción fija. Mezcla: sustancias distintas combinadas sin unión química, en proporciones variables.
      </p>
    </div>
  );
}
...
function ModeloAtomo() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        El número atómico (Z) define cuántos protones (y electrones, si es neutro) tiene un átomo — eso determina de qué elemento se trata. Los neutrones son la diferencia entre el número de masa y Z.
      </p>
    </div>
  );
}
...
function EnlacesQuimicos() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        {tipo === "ionico" ? "En el enlace iónico un átomo transfiere completamente electrones a otro, creando iones de carga opuesta que se atraen (Na⁺ y Cl⁻)." : "En el enlace covalente los átomos comparten pares de electrones, uniéndose mediante esa compartición (como el oxígeno con dos hidrógenos en el agua)."}
      </p>
    </div>
  );
}
...
function EstadosAgregacion() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La energía cinética de las partículas aumenta con la temperatura: en el sólido apenas vibran en su lugar, en el líquido se deslizan entre sí, y en el gas se mueven libremente a gran velocidad.
      </p>
    </div>
  );
}
...
function NaturalezaDual() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        La materia tiene una doble naturaleza: se comporta como partículas discretas (átomos, iones) y también participa en fenómenos energéticos continuos (calor, luz). Ambas vistas son necesarias para explicarla completamente.
      </p>
    </div>
  );
}
...
function TierraSistema() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La Tierra se entiende como un sistema de subsistemas interconectados: lo que pasa en uno (como la quema de combustibles en la atmósfera) afecta a los demás (calentamiento de la hidrosfera, cambios en la biosfera).
      </p>
    </div>
  );
}
...
function CapasAtmosfera() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        La atmósfera se organiza en capas según su altura y temperatura. La troposfera (la más baja) es donde ocurre el clima que vivimos; capas más altas nos protegen de radiación solar dañina.
      </p>
    </div>
  );
}
...
function CadenaTrofica() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        La energía se pierde (como calor) en cada nivel de la cadena trófica — por eso las pirámides se angostan hacia arriba: solo cerca del 10% de la energía pasa de un nivel al siguiente.
      </p>
    </div>
  );
}
...
function ReaccionEstructura() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        En una reacción química los átomos no se crean ni se destruyen: se reorganizan, rompiendo enlaces viejos y formando enlaces nuevos, para dar productos distintos a los reactivos.
      </p>
    </div>
  );
}
...
function OxigenacionAtmosfera() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La atmósfera primitiva de la Tierra casi no tenía oxígeno libre. Los organismos fotosintéticos (cianobacterias) lo fueron liberando durante cientos de millones de años hasta alcanzar el 21% actual.
      </p>
    </div>
  );
}
...
function FotosintesisVisual() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La fotosíntesis usa luz solar para transformar dióxido de carbono y agua en glucosa (alimento) y oxígeno. Sin ella, no habría oxígeno libre en la atmósfera ni base para casi ninguna cadena trófica.
      </p>
    </div>
  );
}
...
function DeterioroAmbiental() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        El deterioro ambiental se analiza con evidencia medible (concentración de gases, pérdida de cobertura vegetal, temperatura), no solo con percepción: cada nivel representa una escala real de daño a los subsistemas terrestres.
      </p>
    </div>
  );
}
...
function InnovacionesAmbientales() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        La tecnología también puede reducir el deterioro ambiental cuando se diseña pensando en el impacto sobre los subsistemas terrestres, no solo en la eficiencia económica inmediata.
      </p>
    </div>
  );
}
...
function CadenaEnergia() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 14, lineHeight: 1.5 }}>
        La energía nunca desaparece, solo cambia de forma. Cada tecnología es, en el fondo, una cadena de transformaciones de energía diseñada para un propósito útil.
      </p>
    </div>
  );
}
...
function PropagacionCalor() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        {modo === "conduccion" && "La conducción transfiere calor por contacto directo, partícula a partícula, a través de un material sólido."}
        {modo === "conveccion" && "La convección mueve calor mediante corrientes: el fluido caliente (menos denso) sube y el frío baja."}
        {modo === "radiacion" && "La radiación transfiere calor como ondas electromagnéticas, sin necesitar ningún medio material — así llega el calor del Sol a la Tierra."}
      </p>
    </div>
  );
}
...
function ConversorCaloriaJoule() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        Una caloría equivale a 4.184 Joules — ambas unidades miden energía (calorífica una, mecánica/eléctrica la otra), y por eso se pueden convertir directamente entre sí.
      </p>
    </div>
  );
}
...
function GasIdeal() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        Si la temperatura y la cantidad de gas no cambian, comprimir el volumen (empujar el punto a la izquierda) aumenta la presión, y expandirlo la reduce — presión y volumen son inversamente proporcionales.
      </p>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <label style={{ display: "block", fontSize: 12.5, color: CI.muted, marginTop: 8 }}>Tiempo transcurrido: <b style={{ color: CI.ink }}>{t}%</b>
        <input type="range" min={0} max={100} value={t} onChange={(e) => setT(+e.target.value)} style={{ width: "100%", accentColor: CI.milpa }} /></label>
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La entropía mide el desorden de un sistema. Con el tiempo, las partículas (inicialmente ordenadas) tienden a dispersarse — la segunda ley de la termodinámica dice que ese desorden nunca disminuye por sí solo.
      </p>
    </div>
  );
}
...
function EficienciaEnergetica() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La eficiencia energética mide qué porcentaje de la energía consumida se convierte en la forma útil deseada. El resto casi siempre se pierde como calor no aprovechado (segunda ley de la termodinámica).
      </p>
    </div>
  );
}
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "center", marginBottom: 10 }}>
        {["síntesis", "descomposición", "desplazamiento", "neutralización"].map((t) => (
          <button key={t} className="tab" style={{ padding: "7px 10px", fontSize: 12, background: resp === t ? (cur.t === t ? "#EAF1E3" : "#F6E3DE") : "#fff" }} onClick={() => setResp(t)}>{t}</button>
        ))}
      </div>
      {resp && <div style={{ textAlign: "center", fontWeight: 800, color: resp === cur.t ? CI.milpaD : CI.rojo }}>{resp === cur.t ? "✓ Correcto" : `✗ Es ${cur.t}`}</div>}
      <div style={{ textAlign: "center", marginTop: 10 }}><button className="tab on" onClick={() => { setIdx((i) => (i + 1) % CASOS.length); setResp(null); }}>Siguiente →</button></div>
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 12, lineHeight: 1.5 }}>
        Síntesis: dos sustancias se combinan en una. Descomposición: una se rompe en varias. Desplazamiento: un elemento reemplaza a otro. Neutralización: ácido + base → sal + agua.
      </p>
    </div>
  );
}
...
function BalanceadorEcuaciones() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 12, lineHeight: 1.5 }}>
        Ajusta los coeficientes con + y − hasta que el número de átomos de cada elemento coincida en ambos lados. La combinación correcta es 4 Fe + 3 O₂ → 2 Fe₂O₃.
      </p>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <label style={{ display: "block", fontSize: 12.5, color: CI.muted, marginTop: 10 }}>Desplazar equilibrio: <b style={{ color: CI.ink }}>{t === 50 ? "en equilibrio" : t > 50 ? "hacia productos" : "hacia reactivos"}</b>
        <input type="range" min={0} max={100} value={t} onChange={(e) => setT(+e.target.value)} style={{ width: "100%", accentColor: CI.milpa }} /></label>
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        En el equilibrio químico ambas reacciones (directa e inversa) ocurren simultáneamente. Cuando las velocidades se igualan (barras iguales), las concentraciones netas dejan de cambiar — es un equilibrio dinámico, no estático.
      </p>
    </div>
  );
}
...
function RedoxTransferencia() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        En toda reacción redox hay transferencia de electrones: quien los pierde se oxida (el hierro), quien los gana se reduce (el cobre). Ambos procesos ocurren siempre juntos, nunca por separado.
      </p>
    </div>
  );
}
...
function EnlacesCarbono() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        El carbono puede compartir 1, 2 o 3 pares de electrones con otro átomo de carbono, formando enlaces simples, dobles o triples. Esa flexibilidad es la razón de la enorme diversidad de compuestos orgánicos.
      </p>
    </div>
  );
}
...
function ClasificadorBiomoleculas() {
  ...
  return (
    <div>
      ...
      <div style={{ textAlign: "center", marginTop: 10 }}><button className="tab on" onClick={() => { setIdx((i) => (i + 1) % CASOS.length); setResp(null); }}>Siguiente →</button></div>
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 12, lineHeight: 1.5 }}>
        Carbohidratos: energía rápida. Lípidos: estructura y reserva energética. Proteínas: estructura y función (enzimas). Ácidos nucleicos: información genética.
      </p>
    </div>
  );
}
...
function RespiracionComparada() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 12, lineHeight: 1.5 }}>
        La respiración aerobia completa (glucólisis + ciclo de Krebs + cadena de electrones) rinde muchísima más energía que la fermentación anaerobia, que se detiene apenas después de la glucólisis.
      </p>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 10 }}>
        ...
        <div style={{ flex: 1, background: "#EAF1E3", border: `1.5px solid ${CI.milpaD}`, borderRadius: 8, padding: 10, fontSize: 12.5 }}><b style={{ color: CI.milpaD }}>Reacción:</b> {c.r}</div>
      </div>
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 12, lineHeight: 1.5 }}>
        Las fuerzas siempre vienen en pares iguales y opuestos, actuando sobre cuerpos distintos — por eso nunca se cancelan entre sí, y por eso es posible avanzar empujando algo en sentido contrario.
      </p>
    </div>
  );
}
...
function GravitacionDistancia() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        La fuerza gravitacional cae muy rápido con la distancia (inversamente proporcional al cuadrado). Duplicar la distancia no reduce la fuerza a la mitad: la reduce a la cuarta parte.
      </p>
    </div>
  );
}
...
function OpticaRayo() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        {medio === "reflexion" ? "En la reflexión, el ángulo de salida (azul) es siempre igual al de entrada (rojo)." : "En la refracción, el rayo se desvía al cambiar de medio (aire→agua), porque la luz viaja más lento en el medio más denso."}
      </p>
    </div>
  );
}
...
function ArquimedesFlota() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        El principio de Arquímedes: un cuerpo sumergido recibe un empuje igual al peso del fluido que desplaza. Si su densidad es menor que la del fluido, ese empuje basta para hacerlo flotar.
      </p>
    </div>
  );
}
...
function RelatividadDivulgativa() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        Según la relatividad especial de Einstein, el tiempo transcurre más lento para quien se mueve a velocidades cercanas a la de la luz. A velocidades cotidianas el efecto es imperceptible, pero es real y medible (y crucial para el GPS).
      </p>
    </div>
  );
}
...
function MillerUreyExperimento() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        El experimento de Miller-Urey (1953) demostró que, bajo condiciones similares a la Tierra primitiva, es posible formar moléculas orgánicas complejas a partir de sustancias inorgánicas simples.
      </p>
    </div>
  );
}
...
function ZoomCelula() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
        Robert Hooke, en 1665, vio por primera vez estas cavidades en un corte de corcho y las llamó "células" por su parecido con las celdas de un monasterio — el microscopio reveló un mundo invisible al ojo humano.
      </p>
    </div>
  );
}
...
function ProcariotaEucariota() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        {sel === "proc" ? "Las procariotas (bacterias) no tienen núcleo definido: su material genético flota libre en el citoplasma." : "Las eucariotas tienen núcleo delimitado por membrana y organelos especializados (como mitocondrias)."}
      </p>
    </div>
  );
}
...
function EmparejamientoBases() {
  const bases = { A: "T", T: "A", G: "C", C: "G" };
  const colores = { A: "#B4432E", T: "#355070", G: "#3D6B35", C: "#D9A526" };
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 12, lineHeight: 1.5 }}>
        En el ADN, A siempre empareja con T, y G siempre con C. Cambia cualquier base de arriba y observa cómo su complementaria (abajo) se ajusta automáticamente — así se copia la información genética.
      </p>
    </div>
  );
}
...
function MitosisMeiosis() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 12, lineHeight: 1.5 }}>
        La mitosis produce 2 células idénticas con el mismo número de cromosomas que la original. La meiosis produce 4 células, cada una con la mitad de cromosomas — la base de los óvulos y espermatozoides.
      </p>
    </div>
  );
}
...
function SeleccionNatural() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        Antes de la contaminación industrial, las polillas claras se camuflaban mejor. Al oscurecerse los troncos, las oscuras (antes muy visibles) pasaron a camuflarse mejor y sobrevivieron más — selección natural documentada en tiempo real.
      </p>
    </div>
  );
}
...
function CaracteristicasVida() {
  ...
  return (
    <div>
      {CARAC.map((c) => (
        <div key={c} onClick={() => toggle(c)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", marginBottom: 4, background: marcadas.includes(c) ? "#EAF1E3" : "#fff", border: `1.3px solid ${CI.ink}`, borderRadius: 8, cursor: "pointer" }}>
          <span style={{ fontSize: 16 }}>{marcadas.includes(c) ? "✅" : "⬜"}</span><span style={{ fontSize: 13 }}>{c}</span>
        </div>
      ))}
      <p style={{ fontSize: 12.5, color: CI.muted, fontStyle: "italic", marginTop: 10, lineHeight: 1.5 }}>
        Estas son las características que, en conjunto, distinguen a los seres vivos de la materia inerte. Ningún rasgo por sí solo basta — es la combinación completa lo que define a la vida.
      </p>
    </div>
  );
}
...
function CasoCard({ c }) {
  ...
  return (
    <div className="card" style={{ borderLeft: "4px solid #B08650" }}>
      ...
      <p style={{ margin: "0 0 8px", fontSize: 16, fontWeight: 800, color: "#2F5233" }}>{c.titulo}</p>
      ...
      {revelado && (
        <>
          {Comp && (
            <div style={{ background: "#FFFDF6", border: "1.5px solid #2E2A21", borderRadius: 10, padding: 12, marginBottom: 12 }}>
              <p style={{ margin: "0 0 8px", fontSize: 11, fontWeight: 800, letterSpacing: 0.5, color: "#355070", textTransform: "uppercase" }}>Prueba tú mismo — muévelo</p>
              <Comp />
            </div>
          )}
          {c.texto && c.planteamiento && <p style={{ margin: "0 0 10px", fontSize: 13.5, lineHeight: 1.55, color: "#2E2A21" }}>{c.texto}</p>}
          {c.pasos.map((p, j) => (
            <p key={j} style={{ margin: "0 0 4px", fontSize: 13, color: "#2E2A21" }}>
              <b style={{ color: "#8a7a5c" }}>Paso {j + 1}: </b>{p}
            </p>
          ))}
          <div style={{ background: "#EAF1E3", borderRadius: 8, padding: "9px 12px", marginTop: 10, fontSize: 13, color: "#2F5233" }}>
            <b>💡 La moraleja: </b>{c.moraleja}
          </div>

          {c.autoverifica && (
            <div style={{ marginTop: 12, background: "#FFF3E0", border: "1.5px solid #B08650", borderRadius: 10, padding: 12 }}>
              <p style={{ margin: "0 0 8px", fontSize: 13, fontWeight: 700, color: "#2E2A21" }}>🎯 Ponte a prueba: {c.autoverifica.pregunta}</p>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {c.autoverifica.opciones.map((op, i) => (
                  <button key={i} className="tab" style={{ padding: "7px 11px", fontSize: 12.5, background: respAuto === i ? (i === c.autoverifica.correcta ? "#EAF1E3" : "#F6E3DE") : "#fff" }} onClick={() => setRespAuto(i)}>{op}</button>
                ))}
              </div>
              {respAuto !== null && (
                <p style={{ marginTop: 8, marginBottom: 0, fontSize: 12.5, fontWeight: 800, color: respAuto === c.autoverifica.correcta ? "#3D6B35" : "#B4432E" }}>
...
function CalculadoraCientifica({ matId }) {
  ...
  const limpiar = () => { setExpr(""); setResultado(null); setError(null); };
  const calcular = () => {
    try {
  ...
  return (
    <div>
      ...
      <div style={{ background: "#FFFDF6", border: "1.5px solid #2E2A21", borderRadius: 10, padding: 14, marginBottom: 10, minHeight: 60 }}>
        <div style={{ fontSize: 18, fontFamily: "monospace", color: "#2E2A21", wordBreak: "break-all", minHeight: 24 }}>{expr || <span style={{ color: "#B4A98A" }}>escribe o toca los botones…</span>}</div>
        {resultado !== null && <div style={{ fontSize: 24, fontWeight: 800, color: "#3D6B35", marginTop: 6 }}>= {resultado}</div>}
        {error && <div style={{ fontSize: 12.5, color: "#B4432E", marginTop: 6 }}>⚠ {error}</div>}
      </div>
      <input
      ...
      {formulas.length > 0 && (
        <div style={{ marginTop: 14 }}>
          <p style={{ fontSize: 11, fontWeight: 800, letterSpacing: 0.5, color: "#8a7a5c", textTransform: "uppercase", margin: "0 0 6px" }}>Fórmulas rápidas de esta materia</p>
    ...
    <div>
      {presets.length > 0 && (
        <div style={{ marginBottom: 12 }}>
          <p style={{ fontSize: 11, fontWeight: 800, letterSpacing: 0.5, color: "#8a7a5c", textTransform: "uppercase", margin: "0 0 6px" }}>Funciones de esta materia</p>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {presets.map((p, i) => (
          ...
          {presetActivo !== null && presets[presetActivo] && (
            <p style={{ fontSize: 12, color: "#355070", fontStyle: "italic", margin: "8px 0 0", lineHeight: 1.5 }}>💡 {presets[presetActivo].nota}</p>
          )}
        </div>
      )}
      <input
      ...
      />
      <div style={{ textAlign: "center", fontSize: 14, fontWeight: 700, color: "#2F5233", marginBottom: 8 }}>f(x) = {texto || "…"}</div>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", background: "#FBF7EC", borderRadius: 10, border: "1.5px solid #2E2A21" }}>
        <line x1={X(xmin)} y1={Y(0)} x2={X(xmax)} y2={Y(0)} stroke="#8a7a5c" strokeWidth="1" />
        <line x1={X(0)} y1={Y(ymin)} x2={X(0)} y2={Y(ymax)} stroke="#8a7a5c" strokeWidth="1" />
        {polylinePts && <polyline points={polylinePts} fill="none" stroke="#3D6B35" strokeWidth="2.4" />}
        {mostrarTangente && yEnPunto !== null && Number.isFinite(yEnPunto) && (
          <>
            <line
              ...
              stroke="#B4432E" strokeWidth="1.8" strokeDasharray="5 3"
            />
            <circle cx={X(puntoX)} cy={Y(yEnPunto)} r="5" fill="#B4432E" stroke="#fff" strokeWidth="1.3" />
          </>
        )}
      </svg>
      <div style={{ display: "flex", gap: 10, marginTop: 10 }}>
        <label style={{ flex: 1, fontSize: 12, color: "#8a7a5c" }}>x mín: <b style={{ color: "#2E2A21" }}>{xmin}</b>
          <input type="range" min={-30} max={0} value={xmin} onChange={(e) => setXmin(+e.target.value)} style={{ width: "100%", accentColor: "#3D6B35" }} /></label>
        <label style={{ flex: 1, fontSize: 12, color: "#8a7a5c" }}>x máx: <b style={{ color: "#2E2A21" }}>{xmax}</b>
          <input type="range" min={0} max={30} value={xmax} onChange={(e) => setXmax(+e.target.value)} style={{ width: "100%", accentColor: "#3D6B35" }} /></label>
      </div>
      <div style={{ marginTop: 10, padding: "10px 12px", background: "#FFFDF6", border: "1.5px solid #2E2A21", borderRadius: 10 }}>
        <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, fontWeight: 700, color: "#2E2A21", cursor: "pointer" }}>
        ...
        {mostrarTangente && (
          <>
            <label style={{ display: "block", fontSize: 12, color: "#8a7a5c", marginTop: 8 }}>Punto x: <b style={{ color: "#2E2A21" }}>{puntoX}</b>
              <input type="range" min={xmin} max={xmax} step={0.5} value={puntoX} onChange={(e) => setPuntoX(+e.target.value)} style={{ width: "100%", accentColor: "#B4432E" }} /></label>
            {derivEnPunto !== null && Number.isFinite(derivEnPunto) && (
              <div style={{ fontSize: 13, color: "#B4432E", fontWeight: 700, marginTop: 4 }}>f'({puntoX}) ≈ {derivEnPunto}</div>
            )}
          </>
        )}
      </div>
      <p style={{ fontSize: 12, color: "#8a7a5c", fontStyle: "italic", marginTop: 8, lineHeight: 1.5 }}>
...
export default function EntrenamaticoV1() {
  ...
  const [vista, setVista] = useState("inicio");     // inicio | practica | diagnostico | resumen | mapaDiag | aprender | casos | herramientas
  const [subHerramienta, setSubHerramienta] = useState("calculadora"); // calculadora | graficador
  ...
  if (!prog) {
    return (
      <div style={{ minHeight: "60vh", display: "grid", placeItems: "center", fontFamily: "system-ui", color: "#8a7a5c", background: "#F6F1E3" }}>
  ...
  const responder = (idx) => {
    ...
    setYGuarda((prev) => {
      ...
      if (!yaDominaba && pDesp.dominado) setAvisoDominio(true); // freno de sobre-práctica
      return n;
    });
  };
  const siguiente = () => {
  ...
  // ---------------- estilos ----------------
  const css = `
    .ent-root { min-height: 100vh; background: #F6F1E3; color: #2E2A21;
      font-family: -apple-system, "Segoe UI", Roboto, system-ui, sans-serif;
      background-image: radial-gradient(#e8e0ca 1px, transparent 1px); background-size: 22px 22px; }
    ...
    .ent-logo { width:40px; height:40px; border-radius:10px; display:grid; place-items:center; background:#3D6B35; color:#F6F1E3; font-size:18px; box-shadow: 2px 2px 0 #2E2A21; }
    .ent-brand h1 { font-family: Georgia, "Times New Roman", serif; font-size:21px; margin:0; letter-spacing:.2px; }
    .ent-brand small { color:#8a7a5c; font-size:12px; display:block; margin-top:-2px; }
    .pill { background:#fff; border:1.5px solid #2E2A21; border-radius:999px; padding:4px 12px; font-weight:700; font-size:13px; box-shadow: 2px 2px 0 #2E2A21; display:flex; align-items:center; gap:5px; }
    .area-tabs { display:flex; gap:10px; margin-bottom:10px; }
    .area-tab { flex:1; padding:11px 8px; border-radius:12px; border:2px solid #2E2A21; background:#fff; font-weight:800; font-size:14px; cursor:pointer; box-shadow:3px 3px 0 #2E2A21; color:#2E2A21; transition:transform .05s; }
    .area-tab.on { background:#2F5233; color:#F6F1E3; }
    .area-tab:active { transform:translate(1px,1px); box-shadow:2px 2px 0 #2E2A21; }
    .tabs { display:flex; gap:6px; margin-bottom:14px; flex-wrap:wrap; }
    .tab { flex:1; min-width:64px; padding:9px 6px; border-radius:10px; border:1.5px solid #2E2A21; background:#fff; font-weight:700; font-size:12.5px; cursor:pointer; box-shadow:2px 2px 0 #2E2A21; color:#2E2A21; }
    .tab.on { background:#3D6B35; color:#F6F1E3; }
    .card { background:#FFFDF6; border:1.5px solid #2E2A21; border-radius:14px; padding:16px; margin-bottom:12px; box-shadow: 3px 3px 0 #2E2A21; }
    .hero { display:flex; gap:14px; align-items:center; }
    .hero-rango { font-family:Georgia,serif; font-size:22px; margin:0 0 2px; }
    .hero-sub { margin:0; color:#8a7a5c; font-size:13px; }
    .lvl-track { height:9px; background:#EDE6D2; border:1px solid #cabf9d; border-radius:99px; overflow:hidden; margin-top:8px; }
    .lvl-fill { height:100%; background:linear-gradient(90deg,#3D6B35,#6FA05C); }
    .eyebrow { font-size:11px; font-weight:800; letter-spacing:1.4px; text-transform:uppercase; color:#8a7a5c; margin:16px 2px 8px; }
    .prop-btn { width:100%; text-align:left; display:flex; gap:12px; align-items:center; background:#FFFDF6; border:1.5px solid #2E2A21; border-radius:12px; padding:12px; margin-bottom:9px; cursor:pointer; box-shadow:2px 2px 0 #2E2A21; }
    .prop-btn:active { transform:translate(1px,1px); box-shadow:1px 1px 0 #2E2A21; }
    .prop-ico { width:38px; height:38px; border-radius:9px; display:grid; place-items:center; font-weight:800; font-size:15px; border:1.5px solid #2E2A21; flex:none; }
    .prop-tit { margin:0; font-size:14.5px; font-weight:800; display:flex; gap:6px; align-items:center; flex-wrap:wrap; }
    .prop-desc { margin:1px 0 5px; font-size:12px; color:#8a7a5c; }
    .dom-track { height:7px; background:#EDE6D2; border-radius:99px; overflow:hidden; }
    .dom-fill { height:100%; }
    .chip { font-size:10.5px; font-weight:800; border:1px solid #2E2A21; border-radius:999px; padding:1px 8px; background:#EDE6D2; }
    .chip.dom { background:#D9A526; }
    .q-text { font-family:Georgia,serif; font-size:18px; line-height:1.45; margin:4px 0 12px; }
    .opt { width:100%; text-align:left; padding:12px 13px; border-radius:10px; border:1.5px solid #2E2A21; background:#fff; margin-bottom:8px; font-size:15px; cursor:pointer; box-shadow:2px 2px 0 #2E2A21; color:#2E2A21; }
    .opt.ok { background:#3D6B35; color:#F6F1E3; }
    .opt.no { background:#B4432E; color:#F6F1E3; }
    .opt.dim { opacity:.55; }
    .fb { border-radius:10px; padding:11px 13px; font-size:13.5px; line-height:1.5; margin-top:4px; border:1.5px solid #2E2A21; }
    .fb.ok { background:#E7F0DF; } .fb.no { background:#F6E3DE; }
    .fb-cuad { display:block; margin-top:7px; font-size:12.5px; color:#5c5138; border-top:1px dashed #b7ab88; padding-top:7px; }
    .btn { padding:12px 16px; border-radius:10px; border:1.5px solid #2E2A21; font-weight:800; font-size:14px; cursor:pointer; box-shadow:2px 2px 0 #2E2A21; }
    .btn:active { transform:translate(1px,1px); box-shadow:1px 1px 0 #2E2A21; }
    .btn-p { background:#3D6B35; color:#F6F1E3; }
    .btn-g { background:#fff; color:#2E2A21; }
    .btn-a { background:#D9A526; color:#2E2A21; }
    .btn-block { width:100%; display:block; }
    .row { display:flex; gap:8px; margin-top:10px; }
    .muted { color:#8a7a5c; font-size:12.5px; }
    ...
    .rs { background:#EDE6D2; border:1px solid #cabf9d; border-radius:10px; padding:10px 6px; }
    .rs .n { font-weight:800; font-size:19px; } .rs .l { font-size:11px; color:#8a7a5c; }
    .foot { text-align:center; margin-top:22px; }
    .link { background:none; border:none; color:#8a7a5c; text-decoration:underline; font-size:12px; cursor:pointer; }
    @media (prefers-reduced-motion: no-preference) { .card { transition: box-shadow .15s ease; } }
  `;

  const colorDom = (d, dominado) => (dominado ? "#D9A526" : d >= 75 ? "#3D6B35" : d >= 40 ? "#6FA05C" : d > 0 ? "#B08650" : "#cabf9d");
      ...
      <div className="ent-wrap">
        ...
        {vista === "inicio" && (
          <>
            ...
            {estadoMat.diagnostico === "pendiente" && (
              <div className="card" style={{ borderColor: "#2E2A21", background: "#FDF6E0" }}>
            ...
            {debil && (estadoMat.props[debil.code]?.vistas ?? 0) > 0 && (
              <button className="prop-btn" style={{ borderColor: "#D9A526", background: "#FDF6E0" }} onClick={() => iniciarPractica(debil)}>
                <div className="prop-ico" style={{ background: "#D9A526" }}>{debil.icono}</div>
                <div style={{ flex: 1 }}>
            ...
            {MAT.propositos.map((pr) => {
              ...
              return (
                <div key={pr.code} className="prop-btn" style={{ cursor: "default", flexWrap: "wrap" }}>
                  <div className="prop-ico" style={{ background: colorDom(st.conf, st.dominado), color: st.conf > 0 ? "#F6F1E3" : "#5c5138", cursor: "pointer" }} onClick={() => iniciarPractica(pr)}>{pr.icono}</div>
                  <div style={{ flex: 1, minWidth: 180, cursor: "pointer" }} onClick={() => iniciarPractica(pr)}>
                    <p className="prop-tit">
                      ...
                      {pr.extra && <span className="chip" style={{ background: "#FBF3DC", borderColor: "#B08650", color: "#8a6a2f" }}>➕ complementario</span>}
                      {pr.interactivo && <span className="chip" style={{ background: "#E7ECF3", borderColor: "#355070", color: "#355070" }}>✦ interactivo</span>}
                  ...
                  <div style={{ display: "flex", gap: 6, width: "100%", marginTop: 4 }}>
                    {pr.interactivo && (
                      <button className="tab" style={{ flex: 1, padding: "7px", fontSize: 12.5 }} onClick={() => iniciarAprender(pr)}>Aprender ✦</button>
                    )}
                    <button className="tab on" style={{ flex: pr.interactivo ? 1 : "none", width: pr.interactivo ? "auto" : "100%", padding: "7px", fontSize: 12.5 }} onClick={() => iniciarPractica(pr)}>Practicar →</button>
                  </div>
                </div>
              );
            })}
            ...
            {CASOS_REALES[matId] && CASOS_REALES[matId].length > 0 && (
              <button className="prop-btn" style={{ borderColor: "#B08650", background: "#FBF3DC", cursor: "pointer" }} onClick={() => setVista("casos")}>
                <div className="prop-ico" style={{ background: "#B08650", color: "#F6F1E3" }}>🌎</div>
                <div style={{ flex: 1 }}>
            ...
            <button className="prop-btn" style={{ borderColor: "#355070", background: "#E6F1FB", cursor: "pointer" }} onClick={() => setVista("herramientas")}>
              <div className="prop-ico" style={{ background: "#355070", color: "#F6F1E3" }}>🧮</div>
              <div style={{ flex: 1 }}>
        ...
        {vista === "herramientas" && (
          <>
            ...
            <div className="card">
              {subHerramienta === "calculadora" ? <CalculadoraCientifica matId={matId} /> : <GraficadorFunciones matId={matId} />}
            </div>
            <button className="btn btn-g btn-block" onClick={() => setVista("inicio")}>← Volver a los propósitos</button>
          </>
        )}
            ...
            {MAT.propositos.map((pr) => {
              ...
              return (
                <div key={pr.code} className="prop-btn" style={{ cursor: "default" }}>
                  <div className="prop-ico" style={{ background: r?.ok ? "#3D6B35" : "#B08650", color: "#F6F1E3" }}>{pr.icono}</div>
                  <div style={{ flex: 1 }}>
          ...
          return (
            <>
              <div className="card">
                ...
                <p style={{ margin: "0 0 12px", fontSize: 14, color: "#5c5138" }}>{propActivo.desc}</p>
                <div style={{ background: "#E7ECF3", borderLeft: "4px solid #355070", borderRadius: 8, padding: "11px 13px", marginBottom: 14 }}>
                  <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: 1, textTransform: "uppercase", color: "#355070", marginBottom: 4 }}>¿Por qué funciona?</div>
                  <div style={{ fontSize: 13.5, lineHeight: 1.55, color: "#2E2A21" }}>{PORQUE_INTERACTIVO[propActivo.interactivo]}</div>
                </div>
                <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: 1, textTransform: "uppercase", color: "#8a7a5c", marginBottom: 8 }}>Interactivo — muévelo</div>
