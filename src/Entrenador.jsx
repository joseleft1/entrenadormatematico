...
// ============================================================================
// ENTRENAMÁTICO v73 · Pensamiento Matemático I-VI + CNEyT I-VI (MCCEMS 2025)
// Compañero digital de los 12 cuadernillos extendidos del ecosistema.
// ACTUALIZACIÓN v73 (22 jul 2026) — FASE 55: SIMULADOR TIPO EXANI-II
...
function PendienteTangente() {
  ...
  return (
    <div>
      ...
      <label style={{ display: "block", fontSize: 12.5, color: CI.muted }}>Mueve el punto (x): <b style={{ color: CI.ink }}>{x0}</b>
        <input type="range" min={-3} max={3} step={0.5} value={x0} onChange={(e) => setX0(+e.target.value)} style={{ width: "100%", accentColor: CI.milpa }} /></label>
      <p style={{ fontSize: 12.5, color: CI.muted, marginTop: 8, lineHeight: 1.5 }}>
...
export default function EntrenamaticoV1() {
  ...
  const css = `
    ...
    .sim-grid { display:grid; grid-template-columns:repeat(5, 1fr); gap:6px; margin:4px 0 8px; }
    .sim-cell { aspect-ratio:1; border-radius:8px; border:1.5px solid ${CI.ink}; background:${CI.papel2}; font-weight:800; font-size:12.5px; cursor:pointer; display:grid; place-items:center; color:${CI.ink}; }
    .sim-cell.resp { background:${CI.surco}; color:${CI.papel}; }
    .sim-cell.actual { border-width:3px; border-color:${CI.azul}; }
          ...
          <>
            ...
            <div className="card" style={{ borderColor: CI.azul, background: "#EDF1F6" }}>
              ...
              <p className="muted" style={{ margin: "6px 0 10px" }}>
                ...
                retroalimentación hasta el final, sin límite de tiempo, y puedes saltar y regresar
                a repasar antes de terminar — como el examen real.
              </p>
          ...
          return (
            <>
              <div className="card">
                ...
                <p style={{ fontSize: 17, fontWeight: 800, margin: "2px 0 10px" }}>Reactivo {sim.idx + 1} de {total}</p>

                <div className="sim-grid">
                  {sim.preguntas.map((_, i) => (
                    <button
                      key={i}
                      className={`sim-cell ${sim.respuestas[i] !== null ? "resp" : ""} ${i === sim.idx ? "actual" : ""}`}
                      onClick={() => irASim(i)}
                    >
                      {sim.respuestas[i] !== null ? "✓" : i + 1}
                    </button>
                  ))}
                </div>
                <p className="muted" style={{ margin: "0 0 12px", fontSize: 12 }}>
                  {respondidas} de {total} respondidas — puedes saltar a cualquier reactivo y regresar cuando quieras.
                </p>
