...
function BotonLeer({ texto, variante = "seccion", etiqueta }) {
  ...
  if (variante === "general") {
    return (
      <button className="tab" style={{ width: "100%", padding: "9px 0", fontSize: 12.5, marginBottom: 10 }} onClick={alternar} aria-label={leyendo ? "Detener lectura" : "Escuchar esta pantalla en voz alta"}>
        {leyendo ? "⏹️ Detener lectura" : "🔊 Escuchar esta pantalla"}
      </button>
    );
  }
  return (
...
// ============================================================================
// ENTRENAMÁTICO v30 · Pensamiento Matemático I-VI + CNEyT I-VI (MCCEMS 2025)
// Compañero digital de los 12 cuadernillos extendidos del ecosistema.
// ACTUALIZACIÓN v30 (14 jul 2026) — FASE 16: LECTURA EN VOZ ALTA
...
// DISEÑO HÍBRIDO (razonado con el usuario):
//  - Botón general "🔊 Escuchar esta pantalla" SIEMPRE visible en Aprender
//    (lee título + descripción + ¿Por qué funciona?). Descubrible sin bucear
//    en menús — clave para quien más lo necesita.
//  - Interruptor en Ajustes ("Lectura en voz alta": Solo botón general /
//    🔊 en cada sección). Al activar "en cada sección", aparece un 🔊 discreto
//    junto a CADA bloque para control fino. Se guarda en entrenador-ui (clave
//    nueva "leer", junto a tema y escala).
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6, gap: 8, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          <BotonLeer texto={`${p.titulo || `Paso ${idx + 1}`}. ${p.linea}. ${p.nota || ""}`} etiqueta="Leer este paso" />
          {modoBtns}
        </div>
      </div>
  ...
  return (
    <div className="card" style={{ borderColor: CI.maiz }}>
      ...
      {especial && (
        <div style={{ marginTop: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
            <p style={{ margin: 0, fontSize: 11, fontWeight: 800, letterSpacing: 0.5, color: CI.rojo, textTransform: "uppercase" }}>⚠️ Especial atención</p>
            {leerActivo && <BotonLeer texto={especial} etiqueta="Leer especial atención" />}
          </div>
      ...
      {vida && vida.length > 0 && (
        <div style={{ marginTop: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
            <p style={{ margin: 0, fontSize: 11, fontWeight: 800, letterSpacing: 0.5, color: CI.milpaD, textTransform: "uppercase" }}>💼 Aplicaciones para la vida</p>
            {leerActivo && <BotonLeer texto={vida.map((c) => `${c.situacion}. ${c.desarrollo}`).join(". ")} etiqueta="Leer aplicaciones para la vida" />}
          </div>
          {vida.map((c, i) => (
      ...
      {dedonde && (
        <div style={{ marginTop: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
            <p style={{ margin: 0, fontSize: 11, fontWeight: 800, letterSpacing: 0.5, color: CI.azul, textTransform: "uppercase" }}>🕰️ De dónde viene</p>
            {leerActivo && <BotonLeer texto={dedonde.texto} etiqueta="Leer de dónde viene" />}
          </div>
      ...
      {cruce && (
        <div style={{ marginTop: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
            <p style={{ margin: 0, fontSize: 11, fontWeight: 800, letterSpacing: 0.5, color: CI.milpa, textTransform: "uppercase" }}>🔗 Cruce de aprendizajes</p>
            {leerActivo && <BotonLeer texto={cruce} etiqueta="Leer cruce de aprendizajes" />}
          </div>
  ...
  const panelSettings = showSettings && (
    <div onClick={() => setShowSettings(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.35)", display: "flex", alignItems: "flex-start", justifyContent: "center", zIndex: 50, padding: "40px 16px", overflowY: "auto" }}>
      <div onClick={(e) => e.stopPropagation()} className="card" style={{ maxWidth: 420, width: "100%", marginBottom: 0 }}>
        ...
        {TTS_DISPONIBLE && (
          <>
            <p style={{ fontSize: 12, fontWeight: 800, letterSpacing: 0.5, color: CI.muted, textTransform: "uppercase", margin: "18px 0 8px" }}>Lectura en voz alta</p>
            <div style={{ display: "flex", gap: 8 }}>
              <button className={`tab ${!leerActivo ? "on" : ""}`} onClick={() => cambiarLeer(false)}>Solo botón general</button>
              <button className={`tab ${leerActivo ? "on" : ""}`} onClick={() => cambiarLeer(true)}>🔊 en cada sección</button>
            </div>
            <p className="muted" style={{ fontSize: 11.5, marginTop: 6, lineHeight: 1.5 }}>Siempre hay un botón «Escuchar esta pantalla» en Aprender. Al activar «en cada sección», aparece un 🔊 junto a cada bloque (¿Por qué funciona?, Más sobre este tema, casos y pasos) para elegir qué leer. Usa la voz del propio equipo — funciona sin internet.</p>
          </>
        )}
      ...
      <div className="ent-wrap">
        ...
        {vista === "aprender" && propActivo && (() => {
          ...
          return (
            <>
              <div className="card">
                <BotonLeer variante="general" texto={textoPantalla} />
