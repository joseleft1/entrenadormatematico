...
function resolverTrig(entrada) {
  ...
  const modo = (partes[0] || "").toLowerCase();
  const nums = partes.slice(1).map(Number);
  const razon = { sen: "seno", seno: "seno", sin: "seno", cos: "coseno", coseno: "coseno" }[modo];
  ...
  return { ok: false, error: "Usa: \"sen OPUESTO HIPOTENUSA\" o \"cos ADYACENTE HIPOTENUSA\". Ejemplo: sen 3 5." };
}
...
function resolverInline(matId, code, texto) {
  ...
  const fn = { jerarquia: resolverJerarquia, lineal: resolverLineal, cuadratica: resolverCuadratica, sistema: resolverSistema, derivada: resolverDerivadaPotencia, densidad: resolverDensidad, cinetica: resolverCinetica, ph: resolverPH, ohm: resolverOhm, regla3: resolverReglaTres, triangulo: resolverTriangulo, trig: resolverTrig, parabola: resolverParabola, mediana: resolverMediana, circunferencia: resolverCircunferencia, optimizacion: resolverOptimizacion, onda: resolverOnda, arquimedes: resolverArquimedes }[tipo];
...
// ============================================================================
// ENTRENAMÁTICO v47 · Pensamiento Matemático I-VI + CNEyT I-VI (MCCEMS 2025)
// Compañero digital de los 12 cuadernillos extendidos del ecosistema.
// ACTUALIZACIÓN v47 (14 jul 2026) — FASE 28: MÓDULO PROPEDÉUTICO (NIVELACIÓN).
...
export default function EntrenamaticoV1() {
  ...
  // propósito (aciertas→sube, fallas→baja). 3 vidas; cada fallo −1 vida; cada 5
  // aciertos +1 vida. 0 vidas = fin. SIN Resolvedor ni ayuda al fallar.
  const HC_VIDAS_INI = 3, HC_ACIERTOS_X_VIDA = 5;
  ...
  const responderHardcore = (i) => {
    ...
    if (ok) {
      aciertos += 1; racha += 1;
      if (aciertos % HC_ACIERTOS_X_VIDA === 0 && vidas < HC_VIDAS_INI) { vidas += 1; vidaGanada = true; }
        ...
        {vista === "inicio" && (
          <>
            <div className="area-tabs" role="tablist" aria-label="Nivel">
              <button role="tab" aria-selected={areaActiva === "prop"} className={`area-tab ${areaActiva === "prop" ? "on" : ""}`} onClick={() => cambiarArea("prop")}>
                🌱 Nivelación
              </button>
            ...
            <div className="card" style={{ borderColor: CI.rojo, background: "#F6E3DE", marginTop: 6 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 6 }}>
                <div style={{ flex: 1, minWidth: 180 }}>
                  ...
                  <p className="muted" style={{ margin: "4px 0 0", fontSize: 12.5 }}>Preguntas de toda la unidad, intercaladas y cada vez más difíciles. 3 vidas (cada 5 aciertos ganas una). A ver hasta dónde llegas — sin ayudas.</p>
                </div>
              ...
              <>
                <div className="card" style={{ borderColor: CI.rojo }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                    <span style={{ fontSize: 20, letterSpacing: 2 }}>{"❤️".repeat(hc.vidas)}{"🖤".repeat(Math.max(0, HC_VIDAS_INI - hc.vidas))}</span>
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
