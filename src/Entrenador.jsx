...
// ============================================================================
// ENTRENAMÁTICO v76 · Pensamiento Matemático I-VI + CNEyT I-VI (MCCEMS 2025)
// Compañero digital de los 12 cuadernillos extendidos del ecosistema.
// ACTUALIZACIÓN v76 (27 jul 2026) — FASE 58: VELOCIDAD + BUG DE OndaInteractiva
// (1) Animación demasiado rápida (reportado): varios componentes barrían su
//     rango completo en menos de 1 segundo (ej. TasaVariacion x₂: 550 ms de
//     ida). Se subió msPorPaso en los 9 componentes (aprox. ×2.2), de forma
//     proporcional para mantener el mismo "sabor" de velocidad relativa entre
//     ellos, no solo en el que se probó.
// (2) BUG REAL en OndaInteractiva (reportado: "solo cambia la frecuencia, no
//     la onda, y por eso no hay animación"). Causa: la curva dibujada
//     y=60−32·sin(2πx/λ) dependía SOLO de λ; la frecuencia únicamente
//     alimentaba el número v=λ·f, nunca el dibujo. Le había puesto el botón ▶
//     a la frecuencia — la ÚNICA variable sin ningún efecto visual posible en
//     una foto fija, porque frecuencia (ciclos/segundo) es una propiedad
//     TEMPORAL que una imagen estática no puede mostrar.
//     Arreglo de fondo, no cosmético: OndaInteractiva ahora es una onda
//     VIAJERA de verdad. Se agregó fase t (tiempo transcurrido) que avanza
//     con el propio requestAnimationFrame; la curva es
//     y=60−32·sin(2π(x−v·t)/λ) con v=f·λ. Esto es correcto físicamente: en un
//     punto fijo de la pantalla, la tasa de oscilación depende SOLO de f, no
//     de λ — verificado analíticamente en la fórmula (el término de fase en
//     un x fijo es 2π·f·t) y en navegador contando cruces por cero de un
//     punto fijo en el tiempo (más cruces con f=8 que con f=1).
//     El control ya no vive en el slider de frecuencia: es un botón dedicado
//     "▶ Ver la onda viajar" que anima el TIEMPO, no un parámetro que rebota.
//     Mismo blindaje que el resto: prefers-reduced-motion apaga el botón solo
//     (verificado), y cancelAnimationFrame limpia el loop al salir del
//     propósito (verificado: 61 f/s animando → 0 al salir, sin fuga).
//     (Nota de proceso: mi primera prueba de "sube la frecuencia, viaja más
//     rápido" comparaba la curva completa antes/después y dio falso negativo
//     por aliasing — a f=8 la onda ya había recorrido más de un ciclo en la
//     ventana de medición, así que la diferencia punto a punto se satura y
//     deja de ser monótona. Hubo que medir oscilaciones en un punto FIJO en
//     el tiempo, que es la definición real de frecuencia. Bug de la prueba,
//     no del código — mismo patrón ya visto con round() y con las fugas de
//     rAF: verificar dos veces antes de reportar.)
// ---
// v75 (27 jul 2026) — FASE 57: MODO ANIMACIÓN EN INTERACTIVOS
...
function TasaVariacion() {
  ...
  return (
    <div>
      ...
      <SliderAnim etiqueta="x₂" valor={x2} setValor={setX2} min={0} max={5} step={1} msPorPaso={240} />
...
function OndaInteractiva() {
  ...
  const [t, setT] = useState(0);          // fase (tiempo transcurrido, en "segundos" de simulación)
  const [animando, setAnimando] = useState(false);
  const vel = r2ci(freq * lambda);
  const W = 300, H = 120;

  // Antes, la curva solo dependía de λ: subir la frecuencia movía el número
  // v=λ·f pero la onda dibujada NUNCA cambiaba (bug reportado — el botón de
  // animar quedaba en la frecuencia, la única variable sin efecto visual).
  // La frecuencia es una propiedad TEMPORAL (ciclos por segundo): una foto fija
  // no puede mostrarla. Por eso ahora la onda viaja de verdad con el tiempo,
  // a la velocidad real v = λ·f — así subir la frecuencia SÍ se ve: la onda
  // viaja más rápido por la pantalla, con la misma λ.
  useEffect(() => {
    if (!animando) return;
  ...
  for (let px = 0; px <= W; px += 2) {
    const x = (px / W) * 12;
    const y = 60 - 32 * Math.sin((2 * Math.PI * (x - vel * t)) / lambda);
    pts.push(`${px},${y}`);
  }
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, margin: "8px 0 4px" }}>
        <button
          ...
          aria-label={animando ? "Pausar la onda viajando" : "Ver la onda viajar"}
          style={{ flex: "0 0 auto", width: 30, height: 30, borderRadius: 8, cursor: "pointer",
        ...
        >{animando ? "⏸" : "▶"}</button>
        <span style={{ fontSize: 12.5, color: CI.muted }}>Ver la onda viajar a la velocidad v = λ·f</span>
      </div>
      <label style={{ display: "block", fontSize: 12.5, color: CI.muted }}>Frecuencia (Hz): <b style={{ color: CI.ink }}>{freq}</b>
      ...
      <p style={{ fontSize: 12.5, color: CI.muted, marginTop: 8, lineHeight: 1.5 }}>Sube la frecuencia y dale a "Ver la onda viajar": con la misma λ, la onda cruza la pantalla más rápido — eso es lo que significa v = λ·f. En el sonido o la luz reales, la velocidad en un medio dado es fija, así que frecuencia y longitud de onda cambian juntas, no por separado.</p>
    </div>
  );
}
...
function CrecimientoPotencias() {
  ...
  return (
    <div>
      ...
      <SliderAnim etiqueta="Exponente" valor={exp} setValor={setExp} min={1} max={8} step={1} msPorPaso={240} />
